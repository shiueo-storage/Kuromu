...
def model_typedefs(vk, model):
    ...
    bitmasks = [
        x for x in vk["registry"]["types"]["type"] if x.get("@category") == "bitmask"
    ]

    basetypes = [
        x for x in vk["registry"]["types"]["type"] if x.get("@category") == "basetype"
    ]
    ...
    # handles
    handles = [
        x for x in vk["registry"]["types"]["type"] if x.get("@category") == "handle"
    ]
...
def model_enums(vk, model):
    ...
    enums_type = [
        x["@name"]
        for x in vk["registry"]["types"]["type"]
    ...
    # create enums
    enums = [
        x for x in vk["registry"]["enums"] if x.get("@type") in ("enum", "bitmask")
    ]
    ...
    for enum in enums:
        ...
        extension = next(
            iter([x for x in VENDOR_EXTENSIONS if name.lower().endswith(x)]), ""
        ).upper()
...
def model_macros(vk, model):
    ...
    # API Macros
    macros = [
        x for x in vk["registry"]["enums"] if x.get("@type") not in ("bitmask", "enum")
    ]
...
def model_funcpointers(vk, model):
    ...
    structs = [
        x for x in vk["registry"]["types"]["type"] if x.get("@category") == "struct"
    ]
...
def model_functions(vk, model):
    ...
    def member_has_str(name):
        c = next(iter([x for x in model["constructors"] if x["name"] == name]), None)
...
def get_extensions_filtered(vk):
    return [
        x
        for x in vk["registry"]["extensions"]["extension"]
...
def generate_py():
    ...
    out_file = path.join(HERE, "output", "vulkan.py")
...
def generate_cdef():
    ...
    out_file = path.join(HERE, "output", "vulkan.cdef.h")
